...
# This code give out the longest string
"""
def longest_consec(strarr, k):
    res = ["".join(strarr[i:i+2]) for i in range(len(strarr))]
    print(res)
    print(max(res, key=len))
"""


# Final code with added constraints
def longest_consec(strarr, k):
    if k <= 0 or k > len(strarr):
        return ''
    else:
        res = ["".join(strarr[i:i+k:]) for i in range(len(strarr))]
